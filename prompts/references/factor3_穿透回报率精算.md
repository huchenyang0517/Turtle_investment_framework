...
> **§17.5 预计算加速**：若 data_pack_market §17.5 存在，直接引用逐年基准结余、AA_incl/AA_excl、CV、λ、λ可靠性。§17.5 为不含 V/X 调整的 base case。LLM 需在此基础上加减调整项：V1/V5 可直接从 §5 现金流量表对应行项读取；V_deduct/X1/X2 需结合年报判断。若 §17.5 缺失，从原始数据手动计算。
...
```
历史均值（含全部年份）= [值] 百万元（AA_incl）
历史均值（剔除非经营收入主导型年份）= [值] 百万元（AA_excl）
若两者差异 > 30%：以 AA_excl 作为外推基准，AA_incl 仅作参考
若两者差异 ≤ 30%：以 AA_incl 作为外推基准
选用基准值 = [值] 百万元（AA = AA_incl 或 AA_excl），理由：[说明]
```
...
```
选用基准值 AA = [步骤7确定值] 百万元
支付率锚定值 = 承诺支付率（若有承诺）；若无承诺则使用近3年支付率均值（与因子2同口径，来源：data_pack_market §6 + §3）
