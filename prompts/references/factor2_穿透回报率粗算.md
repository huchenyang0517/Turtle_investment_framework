...
> 本文件为 Phase 3 执行器的参考文件。Agent 3 在执行因子2时按需加载本文件。
...
```
支付率锚定值 = min(近3年支付率均值, 承诺支付率)
  近3年支付率均值 = [X]%（来源：data_pack_report P11 历史表，同币种计算）
  若无承诺则直接使用近3年均值
  注：支付率 = 年报附注股息总额(报表币种) / 归母净利润(报表币种)，确保使用最新可得年报数据
...
*龟龟投资策略 v0.16_alpha | Factor 2 参考文件*