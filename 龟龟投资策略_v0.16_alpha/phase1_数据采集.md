...
- 逐年列出（最近5年，年份从旧到新排列）
- 缺失科目标注 `⚠️缺失：{科目名}`，并在 §13 Warnings 中生成对应条目
- 数值精度：金额保留到小数点后2位即可，不要求严格对齐
- 若某年度某科目在数据源中以不同名称存在（如 Net Income vs Net Income Applicable to Common Shares），优先使用最接近"归母净利润"语义的科目，并在来源列标注实际使用的字段名
...
| 2 | 行业监管动态 | "{行业} 监管政策 最新" |
| 3 | 行业周期位置 | "{行业} 周期 景气度" |
| 4 | 核心原材料价格趋势（乳企适用） | "原奶价格 走势 {最新年份}" / "中国原料奶 收购价 月度" / "raw milk price China trend" |

**⚠️ 第8.4项触发条件**：仅当目标公司属于乳制品行业时执行（通过 `get_stock_info` 行业分类或公司名称判断）。

采集内容：
- 最近3-5年原奶（生鲜乳）平均收购价走势（按年/季度）
- 最新月度原奶收购价及同比变动
- 行业机构/农业部对原奶价格的展望或预测
- 原奶供需格局变化（如奶牛存栏量、进口大包粉价格）

整理为：
```
| 时间 | 原奶均价（元/kg） | 同比变动 | 来源 |
```
...
6. **Warnings 生成规则**：在采集过程中，遇到以下情况时自动生成 warning 条目并写入 §13：
   - `[数据缺失|高]`：关键科目（净利润、OCF、Capex）连续2年以上缺失
   - `[数据缺失|中]`：非关键科目缺失
   - `[数据异常|高]`：某年度数据与前后年度偏差超过 300%（可能为数据源错误）
   - `[数据异常|中]`：某年度数据与前后年度偏差超过 100%
   - `[审计风险|高]`：搜索到审计意见异常或频繁更换审计师
   - `[治理风险|高]`：搜索到财务造假/重大违规记录
   - `[治理风险|中]`：控股股东大比例质押或持续减持
   - `[结构复杂|中]`：确认为控股公司/多元化集团结构
   - `[跨币种|中]`：报表币种与交易币种不一致（如港元报表但人民币交易）
   若无任何异常，写入：「无异常发现」
7. **年报PDF自动下载**：如果用户未提供年报PDF文件，在所有数据采集完成后执行以下步骤自动下载

---

## 年报PDF自动下载（可选步骤）

> 仅在用户未上传年报PDF时执行。使用 `financial-report-downloader`（技能：`snowball-report-downloader:report-download`）从 `stockn.xueqiu.com` 或 `notice.10jqka.com.cn` 搜索并下载年报。
...
*龟龟投资策略 v0.16_alpha | Phase 1 数据采集 | Agent 1 Prompt*
